...
cdef extern from "seq_sve.h": 
    void scan_sve_impl(float* A, float* B, float* E, float* hidden_states, float* discrete_time_step, float* ssm_state, float* scan_output, int B_size, int D_size, int L_size)
...
def scan_sve(
    ...
    int B_size, int D_size, int L_size
):
    cdef float* A_ptr = <float*>A
    cdef float* B_ptr = <float*>B
    cdef float* E_ptr = <float*>E
    cdef float* hidden_states_ptr = <float*>hidden_states
    cdef float* discrete_time_step_ptr = <float*>discrete_time_step
    cdef float* ssm_state_ptr = <float*>ssm_state
    cdef float* scan_output_ptr = <float*>scan_output
    scan_sve_impl(A_ptr, B_ptr, E_ptr, hidden_states_ptr, discrete_time_step_ptr, ssm_state_ptr, scan_output_ptr, B_size, D_size, L_size)
